...
public class ExceptionTranslator extends ResponseEntityExceptionHandler implements ExceptionTranslation {
    ...
    public Mono<ResponseEntity<Object>> handleAnyException(Throwable ex, ServerWebExchange request) {
        ...
        Exception handledException = ex instanceof Exception ? (Exception) ex : new RuntimeException(ex.getMessage(), ex);
        return handleExceptionInternal(handledException, pdCause, buildHeaders(ex), HttpStatusCode.valueOf(pdCause.getStatus()), request);
    }
